...
let allCaps = runners.map((first_name) => {
    return {'First Name': first_name.first_name.toUpperCase()};
});
...
let ticketPriceTotal = runners.reduce ((total, donate) => {
...
// Problem 1
// The event director needs the email addresses of each runner to send out a registration form.  Log the result into a new array called EmaillAdd.

runners.forEach(function(EmailAdd){
    console.log(EmailAdd.email)
  })

// Problem 2
// The event director needs to have all the runner's last names converted to uppercase because the director BECAME DRUNK WITH POWER. Convert each last name into all caps and log the result

let LastName = runners.map((last_name) => {
    return {'Last Name': last_name.last_name.toUpperCase()};
});
...
// Problem 3
// The medium shirts won't be available for the event due to an ordering issue.  Get a list of runners with medium sized shirts so they can choose a different size. Return an array named MediumShirts that contains information about the runners that have a shirt size of M and log the result
let MediumShirts = runners.filter ((shirt) => {
    return shirt.shirt_size === "M";
});
